...
int employeeFunctionality(Employee E, auto_ptr<Company>& C)
{
	E.getRole();
	int k = 0;
	while (true)
...
int adminFunctionality(Admin A, auto_ptr<Company>& C)
{
	int k = 0;
	while (true)
	{
...
int main() {
	...
	try {
		auto_ptr<Company> C(new Company("employee.txt", "positions.txt", "report.txt"));
		while (true)
		{
			system("cls");
			menu();
